...
class CarouselSliderState extends State<CarouselSlider> with TickerProviderStateMixin {
  ...
  Widget getEnlargeWrapper(Widget? child, {double? width, double? height, double? scale, double rotation = 1}) {
    return Transform.rotate(
      angle: rotation,
      alignment: Alignment.bottomCenter,
      origin: Offset(width ?? 0,height ?? 0),
      child: widget.options.enlargeStrategy == CenterPageEnlargeStrategy.height
          ? SizedBox(child: child, width: width, height: height)
          : Transform.scale(
              scale: scale!,
      ...
      itemBuilder: (BuildContext context, int idx) {
        ...
        return AnimatedBuilder(
          ...
          builder: (BuildContext context, child) {
            ...
            double distortionValueForRotation = 1.0;
            ...
            if (widget.options.scrollDirection == Axis.horizontal) {
              return getCenterWrapper(
                getEnlargeWrapper(
                  ...
                  rotation:
                      (1 - distortionValueForRotation) * options.otherPagesRotation * -0.0174533 * itemOffset.sign,
                ),
              );
            } else {
              return getCenterWrapper(
